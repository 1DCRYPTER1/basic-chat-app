...
function LoginPage({ onLogin, isLoading }) {
  ...
  return (
    <div className="login-root">
      ...
      <motion.div className="login-card" initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }}>
        <div className="login-logo">
          <svg width="40" height="40" viewBox="0 0 24 24" fill="none">
            <path d="M12 2C6.48 2 2 6.48 2 12C2 17.52 6.48 22 12 22C17.52 22 22 17.52 22 12C22 6.48 17.52 2 12 2ZM12 6C13.66 6 15 7.34 15 9C15 10.66 13.66 12 12 12C10.34 12 9 10.66 9 9C9 7.34 10.34 6 12 6ZM12 20C9.33 20 6.91 18.67 5.45 16.63C5.48 14.46 9.87 13.25 12 13.25C14.12 13.25 18.51 14.46 18.55 16.63C17.09 18.67 14.67 20 12 20Z" fill="#60a5fa" />
          </svg>
        </div>
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }}>
          {isLoggedOut ? (
            <>
              ...
              <p className="login-sub" style={{ color: "var(--green)", fontWeight: "600" }}>You have been logged out successfully.</p>
            </>
          ) : (
            <>
        ...
        <motion.button className="login-btn" onClick={handleLoginClick} disabled={isLoading} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
          <span>{isLoggedOut ? "Login back again" : "Continue with Auth0"}</span>
...
function App() {
  ...
  useEffect(() => {
      ...
      if (isAuthenticated && user) {
        ...
        try {
          ...
          if (hasValidStreamImage || hasValidAuth0Image) {
            ...
            if (!hasValidStreamImage && hasValidAuth0Image) {
              await chatClient.current.partialUpdateUser({ id: userId, set: { image: auth0Image } });
            }
          } else {
            setNeedsAvatar(true);
          }
    ...
    return () => {
      if (chatClient.current.userID) {
        setClientReady(false);
        setChannel(null); // Clear channel before disconnecting to prevent runtime errors
  ...
  const deleteChannel = async () => {
    if (!channel) return;
    const confirmDelete = window.confirm("Delete this entire chat for everyone?");
    if (confirmDelete) {
      try {
  ...
  const handleAvatarConfirm = async (url) => {
    try {
      await chatClient.current.partialUpdateUser({ id: chatClient.current.userID, set: { image: url } });
      setNeedsAvatar(false);
      ...
      <div className={`app-root ${showChat ? "mobile-chat-active" : "mobile-list-active"}`}>
        <aside className="sidebar">
          <div className="sidebar-header">
            ...
            <button className="logout-btn" onClick={() => logout({ logoutParams: { returnTo: window.location.origin + "?loggedout=true" } })}>
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M9 21H5a2 2 0 01-2-2V5a2 2 0 012-2h4M16 17l5-5-5-5M21 12H9" /></svg>
            </button>
          </div>
          <UserSearch client={chatClient.current} onSelectUser={async (id) => {
          ...
          <div className="channel-list-wrap">
            ...
            <ChannelList filters={{ members: { $in: [chatClient.current.userID] } }} sort={{ last_message_at: -1 }} Preview={CustomChannelPreview} />
          </div>
        </aside>

        <main className="main-panel">
          {isMobileView && showChat && (
            <button className="mobile-back-btn" onClick={() => setShowChat(false)}>
                  ...
                  <Window>
                    <ChannelHeader Title={({ title }) => (
                      <div style={{ display: 'flex', justifyContent: 'space-between', width: '100%', alignItems: 'center' }}>
                        <span>{title}</span>
                        <button onClick={(e) => { e.stopPropagation(); deleteChannel(); }} className="delete-chat-btn">
